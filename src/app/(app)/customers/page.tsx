...
export default function CustomersPage() {
  ...
  const receivables = useQuery({ queryKey: ["customers", "receivables"], queryFn: () => api.get<{ total: number; count: number }>("/api/customers/receivables") })
  ...
  const cols: Column<Customer>[] = [
    ...
    { key: "balance", header: t("customers.balance"), cell: (c) => c.outstandingBalance > 0.009 ? <Money value={c.outstandingBalance} className="font-medium text-amber-700 dark:text-amber-300" /> : <span className="text-muted-foreground">—</span>, align: "end" },
    { key: "spent", header: t("customers.totalSpending"), cell: (c) => <Money value={c.totalSpending} className="font-medium" />, align: "end", hideOnMobile: true },
  ]

  return (
    <RequirePermission permission="customer.view">
      <PageHeader title={t("customers.title")} description={receivables.data && receivables.data.count > 0 ? t("customers.receivablesSummary", { count: receivables.data.count, total: receivables.data.total.toFixed(2) }) : undefined} actions={can("customer.manage") ? <Button onClick={() => setOpen(true)}><Plus className="h-4 w-4" />{t("customers.add")}</Button> : undefined} />
