...
export const GET = withTenant(async (_req, ctx) => {
  const [user, business, stores] = await Promise.all([
    ...
    prisma.store.findMany({ where: { id: { in: ctx.storeIds }, deletedAt: null }, select: { id: true, name: true, city: true, address: true, phone: true, isActive: true }, orderBy: { createdAt: "asc" } }),
  ])
